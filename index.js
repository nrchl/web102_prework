...
function addGamesToPage(games) {
    ...
    for (const game of games) {
        ...
        // This secction make the whole div clickable, just uncomment the next two lines
        //div.addEventListener('click', function() {
        //    location.href = game.link }, false)
...
// show games that match the search input
function filterMatchedOnly() {
    deleteChildElements(gamesContainer);
...
searchBtn.addEventListener('click', filterMatchedOnly);
...
const unfundedGamesCount = GAMES_JSON.filter((game) => { return game.pledged < game.goal;}).length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalRaised} has been raised for ${GAMES_JSON.length} games. Currently, 
    ${unfundedGamesCount} ${unfundedGamesCount > 1 ? 'games' : 'game'} remains unfunded. 
...
div1.innerHTML = `<p> ${first.name} </p>`;
firstGameContainer.append(div1);

// do the same for the runner up item
let div2 = document.createElement('div');
div2.innerHTML = `<p> ${second.name} </p>`;
